...
var midnightTextTimers = [];
var midnightTextFadeMs = 650;

function setMidnightTextSequence(active) {
  midnightTextTimers.forEach(function(timer) { clearTimeout(timer); });
  midnightTextTimers = [];
  var trailWrap = document.getElementById('text-trail-wrap');
  if (trailWrap) {
    trailWrap.style.transition = '';
    trailWrap.style.opacity = '1';
  }
  if (textTrail) textTrail.setText('nightdrive');
  if (!active || !textTrail) return;

  [
    [1200, 'I like your voice'],
    [5100, 'I know I change so fast _moodswings crazy'],
    [9000, 'But you too..ㅠㅠㅠ'],
    [12900, 'When we date?'],
  ].forEach(function(step) {
    midnightTextTimers.push(setTimeout(function() {
      var trailWrap = document.getElementById('text-trail-wrap');
      if (currentMode !== 'midnight' || !textTrail) return;
      if (!trailWrap) {
        textTrail.setText(step[1]);
        return;
      }
      trailWrap.style.transition = 'opacity ' + (midnightTextFadeMs / 1000) + 's ease';
      trailWrap.style.opacity = '0';
      midnightTextTimers.push(setTimeout(function() {
        if (currentMode !== 'midnight' || !textTrail) return;
        textTrail.setText(step[1]);
        trailWrap.style.opacity = '1';
...
function setMode(mode) {
  ...
  setMidnightTextSequence(mode === 'midnight');
